<div class="wp-block-genesis-custom-blocks-sample" style="cursor: pointer;">
	<div class="gcb-editor-form" aria-label="GCB block form">
		<h3>
			<svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24">
				<path fill="none" d="M0 0h24v24H0V0z"></path>
				<path d="m11.559614,12.629577c0.072421,0.209588 0.172754,0.386832 0.300748,0.531215c0.127742,0.144383 0.279374,0.254611 0.454894,0.330166c0.17552,0.075814 0.367134,0.113592 0.575344,0.113592c0.158672,0 0.298988,-0.014231 0.421198,-0.042176c0.122462,-0.027945 0.237631,-0.065205 0.346263,-0.11178l0,-0.772112l-0.488841,0c-0.072421,0 -0.129,-0.019665 -0.169737,-0.059254c-0.040737,-0.039589 -0.061105,-0.089528 -0.061105,-0.150334l0,-0.649982l1.758722,0l0,2.149184c-0.126737,0.095479 -0.258502,0.177503 -0.395298,0.245813c-0.137047,0.069086 -0.283146,0.125494 -0.438046,0.169741c-0.155152,0.044246 -0.320614,0.076849 -0.495882,0.098067c-0.17552,0.020959 -0.363865,0.03105 -0.565537,0.03105c-0.362105,0 -0.697051,-0.065464 -1.005092,-0.197168c-0.307789,-0.131704 -0.574338,-0.313347 -0.799396,-0.545188c-0.22531,-0.231582 -0.402087,-0.506634 -0.52983,-0.82438c-0.127994,-0.318005 -0.191865,-0.663437 -0.191865,-1.036297c0,-0.379329 0.061105,-0.728642 0.183315,-1.046647c0.12221,-0.317746 0.297228,-0.592022 0.5248,-0.822569c0.227321,-0.230806 0.50242,-0.41012 0.825045,-0.537943c0.322625,-0.12834 0.68297,-0.192511 1.081285,-0.192511c0.206198,0 0.399572,0.017854 0.580876,0.052526c0.181052,0.034931 0.348023,0.0828 0.500911,0.143348c0.152889,0.060548 0.29245,0.132739 0.419438,0.216574c0.126485,0.083835 0.239643,0.174657 0.339473,0.272206l-0.332935,0.520865c-0.031684,0.048904 -0.069403,0.087458 -0.113661,0.115403c-0.044006,0.027945 -0.092286,0.041918 -0.144339,0.041918c-0.067895,0 -0.138052,-0.023288 -0.210473,-0.069863c-0.090526,-0.05589 -0.176023,-0.104277 -0.256239,-0.145159c-0.080468,-0.040883 -0.162696,-0.073744 -0.246432,-0.09936c-0.083737,-0.025616 -0.172,-0.044246 -0.264789,-0.05589c-0.092789,-0.011903 -0.196894,-0.017595 -0.312567,-0.017595c-0.215,0 -0.408625,0.039071 -0.580373,0.117214c-0.172251,0.077625 -0.318602,0.188112 -0.439806,0.329907c-0.121456,0.142313 -0.214497,0.31283 -0.28038,0.511809c-0.065631,0.199238 -0.098321,0.421246 -0.098321,0.665766c0,0.270395 0.03621,0.510257 0.108631,0.719845zm10.562643,0.244261c-0.165462,0.479983 -0.367385,0.914943 -0.612812,1.281075c0.246181,-0.944699 0.354812,-1.943218 0.303766,-2.970718c-0.361602,-4.94628 -4.373671,-8.844878 -9.2734,-8.844878c-3.07965,0 -5.807002,1.541637 -7.500093,3.914123c-0.097064,0.14283 -0.193374,0.287213 -0.2839,0.437289c-0.560759,0.928657 -0.945747,1.917602 -1.169296,2.92673c-0.032187,-0.862675 0.092789,-1.797283 0.363613,-2.774844c-0.852455,0.120837 -1.570629,0.336635 -2.115546,0.644807c0.000251,0 0.000503,0.000259 0.000754,0.000259c-0.482303,0.273241 -0.829823,0.618673 -1.01339,1.035779c-0.000503,0.001294 -0.001257,0.002588 -0.002012,0.003881c-0.025901,0.059771 -0.048784,0.121095 -0.068146,0.183713c-0.023134,0.075555 -0.040988,0.152663 -0.053813,0.230288c-0.003772,0.023029 -0.004275,0.047093 -0.007292,0.07038c-0.006789,0.055373 -0.01383,0.110487 -0.015842,0.167153c-0.000503,0.026393 0.00176,0.05382 0.002012,0.080471c0.000251,0.054338 0.000251,0.108417 0.005281,0.163531c0.002515,0.02691 0.008047,0.054079 0.011316,0.080989c0.007292,0.056149 0.014333,0.112298 0.025901,0.169482c0.005029,0.025616 0.013327,0.052009 0.019363,0.077884c0.014082,0.058995 0.028415,0.118249 0.047023,0.178021c0.007544,0.024064 0.017351,0.048645 0.025649,0.072709c0.021374,0.062618 0.043503,0.125494 0.069655,0.188888c0.009053,0.021735 0.020117,0.043988 0.029672,0.065723c0.029421,0.066758 0.059596,0.133257 0.094298,0.200532c0.009807,0.019148 0.021123,0.038813 0.031433,0.05796c0.037468,0.070639 0.076947,0.141537 0.120199,0.212434c0.010058,0.01656 0.021123,0.03312 0.031181,0.049421c0.046772,0.074779 0.095304,0.149558 0.147859,0.225113c0.009556,0.013455 0.019865,0.027169 0.029421,0.040624c0.056076,0.078919 0.114415,0.157579 0.176526,0.236498c0.00855,0.010868 0.017351,0.021218 0.025901,0.032085c0.06538,0.082283 0.133778,0.164824 0.206198,0.247625c0.006789,0.007763 0.014082,0.015784 0.020871,0.023546c0.07569,0.085905 0.154146,0.171811 0.236625,0.257716c0.005029,0.005434 0.010058,0.010609 0.015088,0.015525c0.085245,0.08901 0.174011,0.177762 0.266801,0.266772c0.003018,0.002846 0.006287,0.005951 0.009304,0.008798c0.095052,0.09108 0.193625,0.182161 0.296222,0.2735c0.001006,0.001294 0.002515,0.00207 0.004023,0.003623c0.104105,0.092633 0.211982,0.185007 0.32338,0.27764c0.000251,0.000259 0.000251,0.000518 0.000503,0.000776c0.112403,0.092892 0.228579,0.185524 0.348023,0.278416c1.79795,1.395702 4.39756,2.740171 7.41535,3.722389c0.551958,0.179832 1.098636,0.340257 1.638272,0.483864c-3.374111,-0.472479 -6.197019,-1.067088 -8.796126,-2.795026c1.056893,4.091885 4.678694,7.107367 8.98774,7.107367c3.025837,0 5.712956,-1.487041 7.41183,-3.788629c1.75998,-0.235722 2.965235,-0.866815 3.276544,-1.879307c0.266298,-0.866298 -0.159427,-1.89121 -1.105928,-2.941997z" fill="black" id="svg_1" transform="rotate(-0.432576 11.9905 11.9115)"></path>
			</svg><?= $args['block_name'] ?> (click here to edit)
		</h3>

		<!-- FIELDS -->
		<?php foreach($args['fields'] as $field): ?>
			
			<?php if ($field['field_type'] == 'text'): ?>
			<div class="genesis-custom-blocks-control width-100">
				<div class="components-base-control css-1wzzj1a e1puf3u3">
					<div class="components-base-control__field css-1kyqli5 e1puf3u2">
						<label class="components-base-control__label css-4dk55l e1puf3u1" for="inspector-text-control-4"><?= $field['field_label'] ?></label>
						<input class="components-text-control__input" type="text" id="inspector-text-control-4" placeholder="" maxlength="" value="<?= $field['value'] ?>">
					</div>
				</div>
			</div>
			<?php endif; ?>
			
			<?php if ($field['field_type'] == 'textarea'): ?>
			<div class="genesis-custom-blocks-control width-100">
				<div class="components-base-control css-1wzzj1a e1puf3u3">
					<div class="components-base-control__field css-1kyqli5 e1puf3u2">
						<label class="components-base-control__label css-4dk55l e1puf3u1" for="inspector-textarea-control-2"><?= $field['field_label'] ?></label>
						<textarea class="components-textarea-control__input css-g8k84m ebk7yr50" id="inspector-textarea-control-2" rows="4" placeholder="" maxlength="" style="width: 100%;"><?= strip_tags($field['value']) ?></textarea>
					</div>
				</div>
			</div>
			<?php endif; ?>
			
			<?php if ($field['field_type'] == 'image'): ?>
			<div class="genesis-custom-blocks-control width-100">
				<div class="components-base-control genesis-custom-blocks-media-controls css-1wzzj1a e1puf3u3">
					<div class="components-base-control__field css-1kyqli5 e1puf3u2">
						<label class="components-base-control__label css-4dk55l e1puf3u1" for="gcb-image-329f2dbb-d722-4f06-a309-72c6c3506849"><?= $field['field_label'] ?></label><img class="gcb-image__img" src="<?= $field['value'] ?>" alt="This has no alt attribute, but its src is <?= $field['value'] ?>">
					</div>
				</div>
			</div>
			<?php endif; ?>
			
			<?php if ($field['field_type'] == 'file'): ?>
			<div class="genesis-custom-blocks-control width-100">
				<div class="components-base-control genesis-custom-blocks-media-controls css-1wzzj1a e1puf3u3">
					<div class="components-base-control__field css-1kyqli5 e1puf3u2">
						<label class="components-base-control__label css-4dk55l e1puf3u1" for="gcb-file-87f2f504-4566-4f7e-97bf-3d1fb1583d05"><?= $field['field_label'] ?></label></label><pre><?= $field['value'] ?></pre>
					</div>
				</div>
			</div>
			<?php endif; ?>
			
			<?php if ($field['field_type'] == 'color'): ?>
			<div class="genesis-custom-blocks-control width-100">
				<div class="components-base-control genesis-custom-blocks-color-control css-1wzzj1a e1puf3u3">
					<div class="components-base-control__field css-1kyqli5 e1puf3u2">
						<label class="components-base-control__label css-4dk55l e1puf3u1" for="gcb-color-color"><?= $field['field_label'] ?></label>
						<div class="components-base-control css-1wzzj1a e1puf3u3">
							<div class="components-base-control__field css-1kyqli5 e1puf3u2">
								<input class="components-text-control__input" type="text" id="gcb-color-color" value="<?= $field['value'] ?>">
							</div>
						</div>
						<div class="components-base-control genesis-custom-blocks-color-popover css-1wzzj1a e1puf3u3">
							<div class="components-base-control__field css-1kyqli5 e1puf3u2">
								<span class="component-color-indicator"></span>
							</div>
						</div>
					</div>
				</div>
			</div>
			<?php endif; ?>
			
			<?php if ($field['field_type'] == 'select'): ?>
			<div class="genesis-custom-blocks-control width-100">
				<div class="components-base-control css-1wzzj1a e1puf3u3">
					<div class="components-base-control__field css-1kyqli5 e1puf3u2">
						<div data-wp-c16t="true" data-wp-component="Flex" class="components-flex components-select-control em5sgkm7 css-1cak69g em57xhy0">
							<div data-wp-c16t="true" data-wp-component="FlexItem" class="components-flex-item em5sgkm3 css-z5z9eh em57xhy0">
								<label data-wp-c16t="true" data-wp-component="Text" for="inspector-select-control-1" class="components-truncate components-text components-input-control__label em5sgkm4 css-3pxyi8 em57xhy0"><?= $field['field_label'] ?></label>
							</div>
							<div class="components-input-control__container css-1t6thiu em5sgkm6">
								<select class="components-select-control__input css-5c5yva e1mv6sxx1" id="inspector-select-control-1">
									<option value="<?= $field['field_label'] ?>" selected><?= $field['value'] ?></option>
									<option value="satu">satu</option>
									<option value="dua">dua</option>
									<option value="tiga">tiga</option>
								</select>
								<span class="components-input-control__suffix css-pvvbxf em5sgkm0">
									<div class="css-1j3xh4d e1mv6sxx0">
										<svg viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" width="18" height="18" role="img" aria-hidden="true" focusable="false"><path d="M17.5 11.6L12 16l-5.5-4.4.9-1.2L12 14l4.5-3.6 1 1.2z"></path></svg>
									</div>
								</span>
								<div aria-hidden="true" class="components-input-control__backdrop css-29yhbg em5sgkm2"></div>
							</div>
						</div>
					</div>
				</div>
			</div>
			<?php endif; ?>
			
			<?php if ($field['field_type'] == 'toggle'): ?>
			<div class="genesis-custom-blocks-control width-100"><div class="components-base-control components-toggle-control css-1wzzj1a e1puf3u3"><div class="components-base-control__field css-1kyqli5 e1puf3u2"><span class="components-form-toggle"><input class="components-form-toggle__input" id="inspector-toggle-control-0" type="checkbox" checked="<?= $field['value'] ?>"><span class="components-form-toggle__track"></span><span class="components-form-toggle__thumb"></span></span><label for="inspector-toggle-control-0" class="components-toggle-control__label"><?= $field['field_label'] ?></label></div></div></div>
			<?php endif; ?>
			
			<?php if ($field['field_type'] == 'checkbox'): ?>
			<div class="genesis-custom-blocks-control width-100"><div class="components-base-control components-checkbox-control css-1wzzj1a e1puf3u3"><div class="components-base-control__field css-1kyqli5 e1puf3u2"><span class="components-checkbox-control__input-container"><input id="inspector-checkbox-control-0" class="components-checkbox-control__input" type="checkbox" checked="<?= $field['value'] ?>" value="1"></span><label class="components-checkbox-control__label" for="inspector-checkbox-control-0"><?= $field['field_label'] ?></label></div></div></div>
			<?php endif; ?>
			
			<?php if ($field['field_type'] == 'radio'): ?>
			<div class="genesis-custom-blocks-control width-100"><div class="components-base-control components-radio-control css-1wzzj1a e1puf3u3"><div class="components-base-control__field css-1kyqli5 e1puf3u2"><label class="components-base-control__label css-4dk55l e1puf3u1" for="inspector-radio-control-0"><?= $field['field_label'] ?></label>

				<div class="components-radio-control__option">
					<input id="inspector-radio-control-0-0" class="components-radio-control__input" type="radio" name="inspector-radio-control-0" value="<?= $field['value'] ?>">
					<label for="inspector-radio-control-0-0"><?= $field['value'] ?></label>
				</div>

			</div></div></div>
			<?php endif; ?>

		<?php endforeach; ?>
		<!-- /FIELDS -->
	</div>
</div>